<?php
    ...
    if($SessionModeAkses=="Pengurus"){
       ...
       //Menghitung Jumlah Pinjaman Yang Menunggak Untuk Anggota
        $JumlahNotifikasi=0;
        $JumlahPeriodeTagihan=0;
        $query_pinjaman_berjalan = mysqli_query($Conn, "SELECT id_pinjaman, tanggal, periode_angsuran FROM pinjaman WHERE status='Berjalan' AND id_anggota='$SessionIdAkses'");
        while ($data = mysqli_fetch_array($query_pinjaman_berjalan)) {
            $id_pinjaman= $data['id_pinjaman'];
            $tanggal= $data['tanggal'];
            $periode_angsuran= $data['periode_angsuran'];
