<?php
    ...
    if (empty($SessionIdAkses)) {
        ...
        if($SessionModeAkses=="Anggota"){
            ...
            $ImageLama = GetDetailData($Conn, 'akses', 'id_akses', $SessionIdAkses, 'image_akses');
        }
        ...
        if (empty($_FILES['image_akses']['name'])) {
            ...
            if($SessionModeAkses=="Anggota"){
                $UpdateFoto = mysqli_query($Conn,"UPDATE anggota SET foto='$namabaru' WHERE id_anggota='$SessionIdAkses'") or die(mysqli_error($Conn)); 
            }else{
                $UpdateFoto = mysqli_query($Conn,"UPDATE akses SET image_akses='$namabaru' WHERE id_akses='$SessionIdAkses'") or die(mysqli_error($Conn)); 
            }
